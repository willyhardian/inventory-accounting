...
<?php include "config.php";
  ...
  if($action == 'edit')
  {
    ...
    $stmt = $conn->prepare("UPDATE kategori SET nama = ?, keterangan = ? WHERE id = ?");
  ...
  else
  {
    ...
    $stmt = $conn->prepare("INSERT INTO kategori (nama, keterangan) VALUES (?, ?)");
  ...
  if($stmt->execute())
  {
    ?>
    <script>
      alert("Berhasil <?php echo $action_text; ?> data jenis <?php echo $nama; ?>");
